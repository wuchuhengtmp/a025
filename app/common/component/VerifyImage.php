...
class VerifyImage extends Controller
{
	protected $config = array(
		'seKey' => 'DHC_VERIFY_IMAGES',   // 验证码加密密钥
	...
	public function check($code, $id = '') {
		...
		// 验证码不能为空
		$di = $this->getDI();
		$secode = $di->get('session')->get($key);
		if (empty($code) || empty($secode)) {
			return false;
		}
		// session 过期
		if (TIMESTAMP - $secode['verify_time'] > $this->expire) {
			$di->get('session')->set($key, null);
			return false;
		}
		if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
//			$this->reset && $di->get('session')->set($key, null);
			return true;
		}
	...
	public function entry($id = '') {
		...
		// 图片宽(px)
		$this->imageW || $this->imageW = $this->length * $this->fontSize * 1.1;
		// 图片高(px)
		...
		if ($this->useZh) { // 中文验证码
			for ($i = 0; $i < $this->length; $i++) {
				...
				imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $this->fontSize * ($i + 1) * 2, $this->fontSize + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
			}
		} else {
			for ($i = 0; $i < $this->length; $i++) {
				$code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet) - 1)];
				imagettftext($this->_image, $this->fontSize, 0, $codeNX, $this->fontSize * 1.2, $this->_color, $this->fontttf, $code[$i]);
		...
		$di = $this->getDI();
		$di->get('session')->set($key . $id, $secode);
